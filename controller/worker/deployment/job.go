...
func (d *DeployJob) newScaleOptions() *ct.ScaleOptions {
...
func (d *DeployJob) oldScaleOptions() *ct.ScaleOptions {
	return &ct.ScaleOptions{
		Timeout:          &d.timeout,
...
func (d *DeployJob) scaleOldFormationDownByOne(typ string, log log15.Logger) error {
	...
	log.Info("scaling old formation up by one", "release.id", d.OldReleaseID, "job.type", typ)
	d.oldFormation.Processes[typ]--
	if err := d.client.Scale(d.oldFormation, d.oldScaleOptions()); err != nil {
		log.Error("error scaling old formation up by one", "release.id", d.OldReleaseID, "job.type", typ, "err", err)
		return err
	}
	return nil
}
